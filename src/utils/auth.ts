...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // Non–api.anthropic.com base (LiteLLM, local gateways): never use Claude.ai
  // OAuth for request auth. Otherwise getAuthHeaders picks Bearer + stale
  // keychain tokens and the proxy gets the wrong credential (user sees
  // "Not logged in" / 401 while ANTHROPIC_API_KEY is set).
...
/**
 * Effective API key from the environment for the Anthropic HTTP client.
 *
 * - `ANTHROPIC_API_KEY` always wins when set (direct Anthropic or any gateway
 *   that expects the same header the Anthropic SDK sends).
 * - Use `CLAUDE_CODE_LLM_GATEWAY_API_KEY` when you want a separate secret name
 *   (e.g. LiteLLM virtual key) without overloading `ANTHROPIC_API_KEY`.
 * - Provider-specific env vars are only read when the matching
 *   `CLAUDE_CODE_USE_*_API_KEY` flag is set, so stray keys in your shell do not
 *   accidentally override Anthropic auth.
 *
 * You must still point traffic at a compatible endpoint: set `ANTHROPIC_BASE_URL`
 * to your gateway (LiteLLM, Cloudflare AI Gateway, etc.). This CLI does not
 * implement native OpenAI / Gemini wire protocols.
 */
export function getResolvedAnthropicApiKeyFromEnv(opts?: {
  /** Bare mode historically honored ANTHROPIC_API_KEY even on homespace. */
  ignoreHomespace?: boolean
}): string | undefined {
  if (!opts?.ignoreHomespace && isRunningOnHomespace()) {
    return undefined
  }
  ...
  const gateway = process.env.CLAUDE_CODE_LLM_GATEWAY_API_KEY
  if (gateway) {
    return gateway
  }
  if (isEnvTruthy(process.env.CLAUDE_CODE_USE_OPENAI_API_KEY)) {
    return process.env.OPENAI_API_KEY
  }
  if (isEnvTruthy(process.env.CLAUDE_CODE_USE_GEMINI_API_KEY)) {
    return (
  ...
  if (isEnvTruthy(process.env.CLAUDE_CODE_USE_DEEPSEEK_API_KEY)) {
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isBareMode()) {
    const bareKey = getResolvedAnthropicApiKeyFromEnv({ ignoreHomespace: true })
    if (bareKey) {
      return { key: bareKey, source: 'ANTHROPIC_API_KEY' }
    }
    if (getConfiguredApiKeyHelper()) {
      return {
  ...
  const apiKeyEnv = getResolvedAnthropicApiKeyFromEnv()
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      ...
      throw new Error(
        'ANTHROPIC_API_KEY, CLAUDE_CODE_LLM_GATEWAY_API_KEY, CLAUDE_CODE_USE_*_API_KEY + provider key, or CLAUDE_CODE_OAUTH_TOKEN is required',
      )
    }
  ...
  // LiteLLM / enterprise proxy: ANTHROPIC_BASE_URL is not api.anthropic.com — the
  // env key (e.g. proxy master key) is intentional; there is no /login OAuth flow.
  if (
    apiKeyEnv &&
