...
import { getCliMascot, type CliMascot } from '../../utils/cliBrand.js';
...
const POSES_BY_MASCOT: Record<CliMascot, Record<ClawdPose, Segments>> = {
  cat: {
    default: {
      r1L: ' ▐',
      r1E: '▛███▜',
...
const APPLE_EYES_BY_MASCOT: Record<CliMascot, Record<ClawdPose, string>> = {
  cat: {
    default: ' ▗   ▖ ',
    'look-left': ' ▘   ▘ ',
    'look-right': ' ▝   ▝ ',
...
const FEET_BY_MASCOT: Record<CliMascot, string> = {
  cat: '  ▘▘ ▝▝  ',
  fox: '  ^   ^  ',
  otter: '  w   w  ',
  bunny: '  n   n  ',
...
const APPLE_FEET_BY_MASCOT: Record<CliMascot, string> = {
  cat: '▘▘ ▝▝',
  fox: '^   ^',
  otter: 'w   w',
  bunny: 'n   n',
...
const MASCOT = getCliMascot();
const POSES = POSES_BY_MASCOT[MASCOT];
const APPLE_EYES = APPLE_EYES_BY_MASCOT[MASCOT];
const FEET = FEET_BY_MASCOT[MASCOT];
const APPLE_FEET = APPLE_FEET_BY_MASCOT[MASCOT];
export function Clawd(t0) {
  ...
  if ($[22] === Symbol.for("react.memo_cache_sentinel")) {
    t11 = <Text color="clawd_body">{FEET}</Text>;
    $[22] = t11;
  } else {
    t11 = $[22];
  }
...
function AppleTerminalClawd(t0) {
  ...
  if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    t7 = <Text color="clawd_body">{APPLE_FEET}</Text>;
    $[6] = t6;
    $[7] = t7;
  } else {
    t6 = $[6];
